{- Start of script -}
{- The game tutorial -}
-- I am not too experienced with the Elm language, so this script may not function properly or at all. Please report bugs in GitHub issues. It is mostly pseudocode right now

-- These are required
#include /Dialogue/Dialogue.elm
#include /Anti-Cheat/Anti-Cheat.elm
#include /Dialogue/Dialogue.elm$Clause="Section#Tutorial"

-- ProTraApp connectivity
do Dialogue("#Tutorial")
function plantTree() {
  -- Tutorial coming soon
}

{-
File info
File type: Elm source code (*.elm)
File version: 1 (2021, Friday, December 10th at 6:22 pm)
Line count (including blank lines and compiler line): 24
-}

{- End of script -}
